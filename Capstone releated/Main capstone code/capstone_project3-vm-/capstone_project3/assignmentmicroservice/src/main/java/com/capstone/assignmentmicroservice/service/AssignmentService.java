...
import com.capstone.assignmentmicroservice.repository.StudentCourseMarksRepository;
...
public class AssignmentService {
    ...
    @Autowired
    private StudentCourseMarksRepository studnetcoursemarksrepo;
    ...
    public List<Integer> getStudentAssignmentMarks(Integer studentId, Integer courseId) {
        // Retrieve the StudentCourseMarks entity based on studentId and courseId
        StudentAssignment studentCourseMarks = studnetcoursemarksrepo.findByStudentIdAndCourseId(studentId, courseId);

        if (studentCourseMarks == null || studentCourseMarks.getMarks().size() <= 1) {
            // Handle case where no marks are found or the list is too short
            return List.of(); // Return an empty list
        }

        // Return the sublist starting from index 1 to skip the 0th index
        return studentCourseMarks.getMarks().subList(0, studentCourseMarks.getMarks().size());
    }
    ...
    public StudentAssignment saveStudentMarks(Integer studentId, Integer courseId, List<Integer> marks) {
        ...
        if (studentCourseMarks == null) {
            studentCourseMarks = new StudentAssignment();
            studentCourseMarks.setStudentId(studentId);
            studentCourseMarks.setCourseId(courseId);
        }

        // Set the marks (including the 0th index)
        studentCourseMarks.setMarks(marks);

        // Save the entity in the database
        return studnetcoursemarksrepo.save(studentCourseMarks);
    }
